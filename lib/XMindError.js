...
class XMindError extends Error {
  constructor(obj = {}) {
    super(obj.message);
    this.errorCode = obj.error_code;
    this.errorName = obj.error_name;
    this.errorData = obj.error_data;
    this.message = 
                this.errorData 
                ? obj.message.replace('{error}', this.errorData.error)
                    .replace('{type}', this.errorData.type)
                    .replace('{key}', this.errorData.key)
                    .replace('{method}', this.errorData.method)
                : obj.message;
    this.obj = obj;
  }
