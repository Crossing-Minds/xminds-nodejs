...
class ApiClient {
    ...
    return function () {
      ...
      if (this.getJwtToken() == '') {
        ...
        return method.apply(this, args).catch(err => {
          if ('errorDetail' in err && 'name' in err.errorDetail && err.errorDetail.name === 'INCORRECT_JWT_TOKEN') {
            return this.loginRefreshToken()
              .then(() => {
                return method.apply(this, args);
              })
   ...
   */
  async loginIndividual(email, password, dbId = null, frontendUserId = null, frontendSessionId = null, opts = {}) {
    let bodyParams = {
      'email': email,
      'password': password
    }
    ...
    if (frontendSessionId)
      bodyParams['frontend_session_id'] = frontendSessionId;
   ...
   * @returns {Promise<Object>} Object represents the jwtToken, refreshToken and database information if resolved. XMindsError if rejected
   */
  async loginService(name, password, dbId = null, frontendUserId = null, frontendSessionId = null, opts = {}) {
    let bodyParams = {
      'name': name,
      'password': password
    }
  ...
  async loginRoot(email, password, opts = {}) {
    return this.loginIndividual(email, password);
  }

  /**
   * Login on a database with your account, using a refresh token and an optional database ID.
   * 
   ...
   * @returns {Promise<Object>} Object represents the jwtToken, refreshToken and database information if resolved. XMindsError if rejected
   */
  async loginRefreshToken(refreshToken = '', dbId = null, opts = {}) {
   ...
   * 
   */
  createAnonymousSessionInteraction(sessionId, itemId, interactionType, timestamp = null, opts = {}) {
    let bodyParams = {};
    bodyParams['interaction_type'] = interactionType;
    if (timestamp)
      bodyParams['timestamp'] = timestamp;
    let path = `/sessions/${sessionId}/items/${itemId}/interactions/`;
    return this.makeRequestWithAutoRefreshToken('POST', path, bodyParams, opts);
  }
  ...
  /**
     ...
     * 
     */
  createAnonymousSessionInteractionsBulk(sessionId, interactions, opts = {}) {
    let path = `/sessions/${sessionId}/interactions-bulk/`;
    return this.makeRequestWithAutoRefreshToken('POST', path, { 'interactions': interactions }, opts);
  }
   ...
   * 
   */
  createAnonymousSessionsInteractionsBulk(interactions, chunkSize = (1 << 12), opts = {}) {
    let path = '/sessions-interactions-bulk/';
    return this.#executeBulk('POST', 'interactions', interactions, chunkSize, path, opts);
  }
   ...
   * 
   */
  listAnonymousSessionsInteractions(amt = null, cursor = null, startTimestamp = null, endTimestamp = null, opts = {}) {
    let queryParams = {}
    if (amt)
      queryParams['amt'] = amt;
    if (cursor)
    ...
    let path = '/sessions-interactions-bulk/' + utils.convertToQueryString(queryParams);
    return this.makeRequestWithAutoRefreshToken('GET', path, null, opts);
  }
   ...
   * 
   */
  resolveAnonymousSession(userId, sessionId, timestamp = null, opts = {}) {
    let bodyParams = {};
    bodyParams['user_id'] = userId;
    if (timestamp)
      bodyParams['timestamp'] = timestamp;
    let path = `/sessions/${sessionId}/resolve/`;
    return this.makeRequestWithAutoRefreshToken('POST', path, bodyParams, opts);
  }
   ...
   * 
   */
  listResolvedAnonymousSessionsByUsers(usersId = null, amt = null, cursor = null, opts = {}) {
    let bodyParams = {};
    if (usersId)
      bodyParams['users_id'] = usersId;
    if (amt)
      bodyParams['amt'] = amt;
    if (cursor)
      bodyParams['cursor'] = cursor;
    let path = '/resolved-sessions/';
    return this.makeRequestWithAutoRefreshToken('POST', path, bodyParams, opts);
  }
