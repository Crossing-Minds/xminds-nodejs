...
class ApiClient {
   ...
   * 
   */
  createOrUpdateRating = function(userId, itemId, rating, timestamp=null) {
    let bodyParams = {
      "rating" : rating
    }
    if(timestamp)
   ...
   * 
   */
  deleteRating = function(userId, itemId) {
    let path = `/users/${userId}/ratings/${itemId}/`;
    return this.makeRequestWithAutoRefreshToken('DELETE', path);
  }
   ...
   * 
   */
  listUserRatings = function(userId, page=1, amt=64) {
   ...
   * 
   */
  createOrUpdateUserRatingsBulk = function(userId, ratings) {
   ...
   * 
   */
  deleteUserRatings = function(userId) {
    let path = `/users/${userId}/ratings/`;
    return this.makeRequestWithAutoRefreshToken('DELETE', path);
  }
   ...
   * 
   */
  createOrUpdateRatingsBulk = function(ratings, chunkSize=(1<<14)) {
   ...
   * 
   * @param {Number} page Page to be listed. Default value is 1
   * @param {Number} cursor Pagination cursor, typically from the next_cursor value from the previous response
   * @returns {Promise.<Object>} Object that represents the list of ratings, if resolved. XMindError if rejected
   * 
   */
  listRatings = function(page=1, cursor=null) {
   ...
   * 
   */
  getRecommendationsItemToItems = function(itemId, filters=null, amt=null, cursor=null) {
    let queryParams = {};
    if(amt)
      queryParams['amt'] = amt;
    if(cursor)
   ...
   * 
   */
  getRecommendationsSessionToItems = function(ratings, userProperties, filters, excludeRatedItems, amt, cursor) {
    let bodyParams = {};
    if(ratings)
   ...
   * 
   */
  getRecommendationsUserToItems = function(userId, filters, excludeRatedItems, amt, cursor) {
    let queryParams = {};
    if(amt)
      queryParams['amt'] = amt;
    if(cursor)
