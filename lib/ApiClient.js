...
class ApiClient {
   ...
   * 
   */
  #executeBulk(objPropertyName, array, chunkSize, endpoint, opts = {}) {
    return new Promise((resolve, reject) => {
      Promise.all(utils.chunk(array, chunkSize)
        .map(chunkedArr => {
          let bodyParams = {};
          bodyParams[objPropertyName] = chunkedArr;
          this.makeRequestWithAutoRefreshToken('PUT', endpoint, bodyParams, opts)
            .then(resolve)
            .catch(reject)
        })
      )
  ...
  createOrUpdateUsersBulk(users, chunkSize = (1 << 10), opts = {}) {
    ...
    return this.#executeBulk('users', users, chunkSize, path, opts);
  }
  ...
  createOrUpdateItemsBulk(items, chunkSize = (1 << 10), opts = {}) {
    let path = '/items-bulk/';
    opts.timeout = opts.timeout || 60;
    return this.#executeBulk('items', items, chunkSize, path, opts);
  }
  ...
  createOrUpdateRatingsBulk(ratings, chunkSize = (1 << 14), opts = {}) {
    ...
    opts.timeout = opts.timeout || 60;
    return this.#executeBulk('ratings', ratings, chunkSize, path, opts);
  }
